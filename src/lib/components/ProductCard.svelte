...
>
	...
	<div class="aspect-[2/3] bg-muted flex items-center justify-center overflow-hidden">
		{#if product.coverImage}
			...
			/>
		{:else}
			<svg class="w-12 h-12 sm:w-16 sm:h-16 text-muted-foreground opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
		...
		<div class="flex items-center justify-between mt-auto pt-3">
			...
			{#if product.rating > 0}
				<div class="flex items-center gap-1">
					...
					<span class="text-xs sm:text-sm">{product.rating.toFixed(1)}</span>
				</div>
			{/if}
		</div>
	</div>
