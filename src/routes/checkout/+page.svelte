<script>
	...
	const handleCheckout = () => {
		return async ({ result, formElement }) => {
			...
			if (result.type === "success" && result.data?.authorization_url) {
		...
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
			...
			<div class="lg:col-span-2 space-y-6">
				...
				<div class="bg-card border border-border rounded-lg p-6">
					...
					<div class="space-y-3">
						...
						<div class="flex justify-between">
							<span class="text-muted-foreground">Phone (M-Pesa):</span>
							<span class="text-foreground font-medium">{data.user.phone}</span>
						</div>
					</div>
				</div>
			...
			<div class="lg:col-span-1">
				<div class="bg-card border border-border rounded-lg p-6 sticky top-24">
					...
					<!-- Checkout Form -->
					<form method="POST" action="?/initiate" use:enhance={handleCheckout}>
						...
						<button
							...
							class="w-full px-6 py-3 bg-primary text-primary-foreground font-semibold rounded-lg hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed mb-3"
						>
					...
					<div class="mt-6 pt-6 border-t border-border">
						...
						<div class="flex items-start gap-3 text-sm text-muted-foreground">
							...
							<div>
								...
								<p>Download your purchases immediately after successful payment.</p>
							</div>
						</div>
					</div>
				</div>
