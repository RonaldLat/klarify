<script>
	...
	import { Zap, Download, Clock, Check, Headphones, Book, AlertCircle, Loader2, ChevronDown, X } from '@lucide/svelte';
	...
	async function downloadCompleteAudiobook() {
		...
		console.log(`📦 Starting download: ${audioData.chapters.length} chapters in ${totalBatches} batches`);
		...
		try {
			...
			for (let batchIdx = 0; batchIdx < totalBatches; batchIdx++) {
				...
				while (true) {
					...
					const overallProgress = ((batchesCompleted + currentBatchProgress) / totalBatches) * 100;
				...
				console.log(`✅ Batch ${batchIdx + 1} complete: ${formatBytes(batchBytes)}`);
			...
			console.log(`📦 Combining ${allChunks.length} chunks (${formatBytes(totalReceivedBytes)})`);
			...
			console.log(`✅ Download complete: ${filename} (${formatBytes(totalReceivedBytes)})`);
										...
										<div>
											...
											<div class="text-xs text-muted-foreground">
												{#if isDownloadingComplete}
													{downloadStatus || `Processing batch ${currentBatch}/${totalBatches}... ${downloadProgress}%`}
												{:else if audioData.chapters.length > BATCH_SIZE}
													Downloaded in {Math.ceil(audioData.chapters.length / BATCH_SIZE)} batches (~{Math.ceil(audioData.chapters.length / BATCH_SIZE)}min)
												{:else}
								...
								>
									<div class="flex items-center gap-3">
										...
										<div class="text-left">
											...
											<div class="text-xs text-muted-foreground">Instant downloads for each chapter</div>
										</div>
									</div>
									...
									<div class="border-t border-border">
										<div class="p-4 bg-muted/30">
											<p class="text-xs text-muted-foreground mb-4">
												💡 Tip: Individual chapters download instantly. Use this if you want specific chapters.
											</p>
							...
							<div class="p-4 bg-gradient-to-r {isSummary ? 'from-amber-500/10 to-amber-500/5 border-amber-500/20' : 'from-purple-500/10 to-purple-500/5 border-purple-500/20'} border rounded-lg">
								<div class="flex items-center justify-between mb-3">
									<div class="flex items-center gap-3">
										...
										<div>
											<div class="font-semibold text-foreground">{isSummary ? 'Audio Summary' : 'Audiobook'}</div>
											<div class="text-xs text-muted-foreground">Single audio file - instant download</div>
										</div>
									</div>
								</div>
								<button
					...
					<div class="text-sm">
						...
						<ul class="space-y-1.5 text-muted-foreground">
							...
							{#if hasMultipleChapters && audioData.chapters.length > BATCH_SIZE}
								<li>• Large audiobooks download in batches (no extra charge)</li>
