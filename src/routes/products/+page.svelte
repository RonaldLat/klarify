<script>
    ...
    let { data } = $props();
    // console.log(data) // Keeping this commented out, but available for debugging
    ...
    // Build query string helper - UPDATED to include sortBy, filterBy, and typeFilter
    function buildQuery(page) {
        ...
        if (data.typeFilter) params.set("type", data.typeFilter); // Changed to typeFilter
        if (data.sortBy) params.set("sort", data.sortBy); // Added for state persistence
        if (data.filterBy) params.set("filter", data.filterBy); // Added for state persistence
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
                ...
                <div class="flex gap-2 flex-wrap">
                    <a
                        href={buildQuery(1).replace(/&type=[^&]*/g, "")}
                    ...
                    <a
                        href={buildQuery(1).replace(/&type=[^&]*/g, "") + "&type=EBOOK"}
                        class="flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors
							{data.typeFilter === 'EBOOK'
                                ? 'bg-primary text-primary-foreground'
                                : 'bg-secondary text-secondary-foreground hover:bg-secondary/80'}"
                    >
                        <Book class="w-4 h-4" />
                    ...
                    <a
                        href={buildQuery(1).replace(/&type=[^&]*/g, "") + "&type=AUDIOBOOK"}
                        class="flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors
							{data.typeFilter === 'AUDIOBOOK'
                                ? 'bg-primary text-primary-foreground'
                                : 'bg-secondary text-secondary-foreground hover:bg-secondary/80'}"
                    >
                    ...
                    <a
                        href={buildQuery(1).replace(/&type=[^&]*/g, "") + "&type=MAGAZINE"}
                        class="flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors
							{data.typeFilter === 'MAGAZINE'
                                ? 'bg-primary text-primary-foreground'
                                : 'bg-secondary text-secondary-foreground hover:bg-secondary/80'}"
                    >
                    ...
                    <nav class="mt-8 sm:mt-12" aria-label="Pagination">
                        ...
                        <div class="hidden sm:flex items-center justify-center gap-1">
                            ...
                            {#each pageNumbers as pageNum}
                                {#if pageNum === "..."}
                                    <span class="px-4 py-2 text-muted-foreground">...</span>
                                {:else if pageNum === data.currentPage}
                                    <span
                                        class="px-4 py-2 rounded-md bg-primary text-primary-foreground text-sm font-medium cursor-default"
                                        aria-current="page"
                                    >
                                        {pageNum}
                                    </span>
